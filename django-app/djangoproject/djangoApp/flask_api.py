...
#read endpoints 
    ...
    def get_story(self, story_id, include_pages=False):
        try:
            ...
            response = requests.get(f"{self.url}/stories/{story_id}", params = params, timeout=10)
...
# writing endpoints
    ...
    def create_story(self, title, description='', status='draft', author_id = None, tags=None):
        try:
            ...
            response = requests.post(f"{self.url}/stories", json=data, headers=self._get_head(include_auth=True), timeout=10)
            result=self._handle_response(response)
            return result.get('story') if result else None
        except Exception as e:
    ...
    def update_story(self, story_id, **kwargs):
        try: 
            response = requests.put(f"{self.url}/stories/{story_id}", json=kwargs, headers=self._get_head(include_auth=True), timeout=10)
            result =self._handle_response(response)
            return result.get('story') if result else None
        except Exception as e:
    ...
    def delete_story(self, story_id):
        try: 
            response = requests.delete(f"{self.url}/stories/{story_id}", headers=self._get_head(include_auth=True), timeout=10)
    ...
    def create_page(self, story_id, text, is_ending=False, ending_label=None):
        try:
            ...
            response = requests.post(f"{self.url}/stories/{story_id}/pages", json=data, headers=self._get_head(include_auth=True), timeout=10)
            result=self._handle_response(response)
    ...
    def update_page(self, page_id, **kwargs):
        try: 
            response = requests.put(f"{self.url}/pages/{page_id}", json=kwargs, headers=self._get_head(include_auth=True), timeout=10)
            result =self._handle_response(response)
            return result.get('pages') if result else None
        except Exception as e:
    ...
    def delete_page(self, page_id):
        try: 
            response = requests.delete(f"{self.url}/pages/{page_id}", headers=self._get_head(include_auth=True), timeout=10)
            return response.status_code==200
        except Exception as e:
    ...
    def create_choice(self, page_id, text, next_page_id):
        try:
            ...
            response = requests.post(f"{self.url}/pages/{page_id}/choices", json=data, headers=self._get_head(include_auth=True), timeout=10)
            result=self._handle_response(response)
